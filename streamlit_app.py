...
from app.predict import two_stage_prediction as two_stage_predict
...
if st.button("Predict"):
    try:
        ...
        st.write("Approval probability : {:2%}".format(res['approval_proba']))
        if res['approval'] == 1:
            st.success("✅ Approved")
            st.write(f"Predicted loan amount (or interest): {res['regression_prediction']:.2f}")
        else:
            st.error("❌ REJECTED")
